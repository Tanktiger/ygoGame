...
foreach ($cardlinks as $link) {
	...
	if (isset($div)) {
		...
		echo 'get Pics: ' . PHP_EOL;
			//erstmal laden der Links und dann nen extra Crawler der die aus der DB holt??
		foreach ($singleCardLinks as $singleCard) {
			echo '.';
			$curlResult = setCurl($singleCard);
			@$dom->loadHTML($curlResult);
